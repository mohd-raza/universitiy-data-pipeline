...
dag1 = DAG(
    dag_id="get_university_api_send_sqlite",
    start_date=airflow.utils.dates.days_ago(1),
    schedule_interval=None
)

dag2 = DAG(
    dag_id="get_sqlite_data_send_s3",
    start_date=airflow.utils.dates.days_ago(1),
    schedule_interval=None
)
...
def _fetch_and_save():
    ...
    csv_file = os.path.join(main_path, "data_university.csv")
...
def _read_json_file():
    main_path = "/tmp/json_file"
    csv_file_path = os.path.join(main_path, "data_university.csv")
    json_file_path = os.path.join(main_path, "data_university.json")
...
def _send_to_sqlite():
    ...
    sqlite_db_path = '/mnt/c/Users/mohdr/Documents/airflow/databases/university.db'

    # Ensure the directory exists
    pathlib.Path('/tmp/sqlite').mkdir(parents=True, exist_ok=True)
...
def _fetch_sqlite_save_csv(file_path, csv_path):
    sqlite_db_path = '/mnt/c/Users/mohdr/Documents/airflow/databases/university.db'
...
send_to_sqlite = PythonOperator(
    ...
    python_callable=_send_to_sqlite,
    dag=dag1
)

trigger_dag2 = TriggerDagRunOperator(
    task_id="trigger_dag2", 
    trigger_dag_id="get_sqlite_data_send_s3",
    dag=dag1
)

fetch_sqlite_save_csv = PythonOperator(
    task_id="fetch_sqlite_save_csv",
    python_callable=_fetch_sqlite_save_csv,
...
fetch_and_save >> read_json_file >> wait_for_json >> send_to_sqlite >> trigger_dag2

fetch_sqlite_save_csv >> wait_for_csv >> send_csv_s3
